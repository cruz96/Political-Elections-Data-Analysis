...
class cleanName(beam.DoFn):
  def process(self, element):
    ...
    if(output != "" and output != "BLANK" and output!= "VOTE BLANK" and output!="VOTE VOID" and output!="SCATTERING" and output !="VOTE OVER" and output != "VOTE/SCATTERING BLANK" and output != "OTHER"):
        ...
        new_record = {'candidate':output, 'original':record.get('candidate')}
        return [new_record]
    else:
        return
...
with beam.Pipeline('DirectRunner', options=opts) as p:
    ...
    query_results = p | 'Read from BigQuery' >> beam.io.Read(beam.io.BigQuerySource(query='SELECT year, state, state_po, state_fips, state_cen, state_ic, office, candidate, party, writein, candidatevotes, totalvotes, version FROM [not-sure-230422:Election_Outcome.President_Result_By_State] where year >= 2012 LIMIT 100'))
