...
class cleanName(beam.DoFn):
  def process(self, element):
    ...
    if(output != "" and output != "BLANK" and output!= "BLANK VOTE" and output!="VOID VOTE" and output!="SCATTERING" and output !="OVER VOTE" and output != "BLANK VOTE/SCATTERING"):
        ...
        new_record = {'name':output, 'original':record.get('candidate')}
        return [new_record]
    else:
        return
...
with beam.Pipeline('DirectRunner', options=opts) as p:
    ...
    query_results = p | 'Read from BigQuery' >> beam.io.Read(beam.io.BigQuerySource(query='SELECT year, state, state_po, county, FIPS, office, candidate, party, candidatevotes, totalvotes, version FROM [not-sure-230422:Election_Outcome.President_Result_By_County] where year >= 2012 LIMIT 100'))
