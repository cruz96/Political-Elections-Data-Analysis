...
class cleanName(beam.DoFn):
  def process(self, element):
    ...
    if(output != "" and output != "BLANK" and output!= "BLANK VOTE" and output!="VOID VOTE" and output!="SCATTERING" and output !="OVER VOTE" and output != "BLANK VOTE/SCATTERING"):
        new_record = {'year':year, 'state':record.get('state'), 'state_po':record.get('state_po'), 'state_fips':record.get('state_fips'), 'state_cen':record.get('state_cen'), 'state_ic':record.get('state_ic'), 'office':record.get('office'), 'stage':record.get('stage'), 'special':record.get('special'), 'candidate': output, 'party':record.get('party'), 'writein':record.get('writein'), 'candidatevotes':record.get('candidatevotes'), 'totalvotes':record.get('totalvotes'), 'version':record.get('version')}
        return [new_record]
    else:
        return
...
with beam.Pipeline('DataflowRunner', options=opts) as p:
    ...
    table_schema = 'year:DATE,state:STRING,state_po:STRING,state_fips:INTEGER,state_cen:INTEGER,state_ic:INTEGER,office:STRING,stage:STRING,special:BOOLEAN,candidate:STRING,party:STRING,writein:BOOLEAN,candidatevotes:INTEGER,totalvotes:INTEGER,version:INTEGER'
